...
export class GameEngine {
    ...
    spawnAsteroidInternal(first?: boolean) {
        ...
        g.add(mesh);
        const a = Math.random()*Math.PI*2;
        const dist = first ? 130 : (200+Math.random()*400); // FIXED: Closer asteroids (200-600)
        ...
        g.userData = { rotSpeed: new THREE.Vector3((Math.random()-0.5)*2, (Math.random()-0.5)*2, (Math.random()-0.5)*2) };
    ...
    buyAsteroid(m: number) { if(this.state.resources >= 500*m) { this.state.resources -= 500*m; for(let i=0; i<m; i++) this.spawnAsteroidInternal(); } }
