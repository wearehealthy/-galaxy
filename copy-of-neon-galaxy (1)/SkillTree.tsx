...
export const SkillTree: React.FC<SkillTreeProps> = ({ stats, actions, onClose }) => {
  ...
  return (
    <div className="absolute inset-0 bg-black/90 z-50 flex items-center justify-center p-8 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
      <div className="bg-slate-950 border border-cyan-500/50 rounded-lg w-full max-w-4xl h-[90vh] flex flex-col shadow-[0_0_50px_rgba(6,182,212,0.15)] relative">
        ...
        <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex flex-col gap-4">
                <div className="text-cyan-400 font-bold border-b border-cyan-900 pb-2 mb-2 flex items-center gap-2"><Rocket size={16}/> LOGISTICS</div>
            ...
            <div className="flex flex-col gap-4">
                <div className="text-red-400 font-bold border-b border-red-900 pb-2 mb-2 flex items-center gap-2"><Sword size={16}/> OFFENSE</div>
                <div className="flex flex-wrap gap-4">
            ...
            <div className="flex flex-col gap-4">
                <div className="text-blue-400 font-bold border-b border-blue-900 pb-2 mb-2 flex items-center gap-2"><Shield size={16}/> DEFENSE</div>
                <div className="flex flex-wrap gap-4">
             ...
             <div className="col-span-1 md:col-span-3 mt-8">
                <div className="text-emerald-400 font-bold border-b border-emerald-900 pb-2 mb-2 flex items-center gap-2"><Globe size={16}/> OPERATIONS</div>
                <div className="flex gap-4">
                    <button onClick={() => actions.buyAsteroid(multiplier)} disabled={stats.resources < 500*multiplier} className="flex items-center gap-4 px-6 py-4 bg-slate-800 rounded border border-slate-600 hover:border-emerald-400 disabled:opacity-50">
                        <Globe className="text-emerald-400" />
                        <div className="text-left"><div className="font-bold text-sm">Spawn Asteroid</div><div className="text-xs text-amber-400">{(500*multiplier).toLocaleString()} RES</div></div>
                    </button>
                    <button onClick={() => actions.repairHull()} disabled={stats.resources < 500 || stats.hullIntegrity >= 100} className="flex items-center gap-4 px-6 py-4 bg-slate-800 rounded border border-slate-600 hover:border-emerald-400 disabled:opacity-50">
                        <RefreshCw className="text-emerald-400" />
                        <div className="text-left"><div className="font-bold text-sm">Repair Hull</div><div className="text-xs text-amber-400">500 RES</div></div>
                    </button>
                </div>
            </div>
        </div>
